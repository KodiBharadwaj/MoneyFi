...
public interface IncomeRepository extends JpaRepository<IncomeModel, Long> {
    ...
    public List<IncomeModel> findIncomesOfUser(Long userId);
    ...
    @Query("SELECT MONTH(i.date) AS month, SUM(i.amount) AS total " +
            ...
            "ORDER BY month ASC")
    public List<Object[]> findMonthlyIncomes(Long userId, int year, boolean deleteStatus);
    ...
    public IncomeModel getIncomeBySourceAndCategory(Long userId, String source, String category);

    @Query("select i.amount from IncomeModel i where i.id = :incomeId")
    public Double getIncomeByIncomeId(Long incomeId);
}
