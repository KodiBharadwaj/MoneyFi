...
import com.moneyfi.user.repository.auth.*;
...
import com.moneyfi.user.service.user.dto.emaildto.GmailSyncIncreaseRequestDto;
...
public class UserCommonServiceImpl implements UserCommonService {
    ...
    public Map<Boolean, String> sendReferenceRequestNumberEmail(String requestStatus, String email) {
        ...
        if (requestStatus.equalsIgnoreCase(RequestReason.ACCOUNT_UNBLOCK_REQUEST.name())) {
            ...
            applicationEventPublisher.publishEvent(new NotificationQueueDto(NotificationQueueEnum.SEND_REFERENCE_NUMBER_TO_USER_MAIL.name(), userProfile.getName() + "<|>" + email + "<|>" + "account unblock" + "<|>" + referenceNumber));
            ...
            applicationEventPublisher.publishEvent(new NotificationQueueDto(NotificationQueueEnum.SEND_REFERENCE_NUMBER_TO_USER_MAIL.name(), userProfile.getName() + "<|>" + email + "<|>" + "account retrieval" + "<|>" + referenceNumber));
    ...
    public void accountReactivateRequestByUser(AccountRetrieveRequestDto requestDto) {
        ...
        UserAuthModel userAuth = userRepository.getUserDetailsByUsername(requestDto.getUsername().trim()).orElseThrow(() -> new ResourceNotFoundException(USER_NOT_FOUND));
    ...
    public void nameChangeRequestByUser(NameChangeRequestDto requestDto) {
        ...
        UserAuthModel userAuth = userRepository.getUserDetailsByUsername(requestDto.getEmail().trim()).orElseThrow(() -> new ResourceNotFoundException(USER_NOT_FOUND));
        if (!user.getReferenceNumber().equals(requestDto.getReferenceNumber()))
            throw new ScenarioNotPossibleException(INCORRECT_REFERENCE_NUMBER);
        user.setRequestStatus(RaiseRequestStatus.SUBMITTED.name());
        ContactUs savedRequest = contactUsRepository.save(user);
    ...
    public void sendContactUsDetailsToAdmin(HelpCenterContactUsRequestDto requestDto) {
        ...
        applicationEventPublisher.publishEvent(new NotificationQueueDto(NotificationQueueEnum.CONTACT_US_HELP_CENTER_MAIL.name(), email + "<|>" + requestDto.getPhoneNumber() + "<|>" + requestDto.getName() + "<|>" + requestDto.getDescription()));
    }
    ...
    public ResponseEntity<String> blockOrDeleteAccountByUserRequest(String username, AccountBlockOrDeleteRequestDto request) {
        ...
        Optional<OtpTempModel> otpTempModel = otpTempRepository.getOtpTempDetails(username, deactivationType, LocalDateTime.now());
        if(otpTempModel.isPresent()){
            OtpTempModel response = otpTempModel.get();
    ...
    public Boolean getUsernameByDetails(ForgotUsernameDto userDetails) {
        ...
        if (StringUtils.isNotBlank(username)) {
            ...
            userAuthHist.setReasonTypeId(reasonCodeIdAssociation.get(ReasonEnum.FORGOT_USERNAME));
            userAuthHist.setComment("Forgot my username, fetched with my known personal values");
            userAuthHist.setUpdatedBy(user.getId());
            userAuthHistRepository.save(userAuthHist);
        }
        applicationEventPublisher.publishEvent(new NotificationQueueDto(NotificationQueueEnum.FORGOT_USERNAME_REQUEST_MAIL.name(), username));
        return true;
    }
    ...
    public void userRequestToIncreaseGmailSyncDailyCount(GmailSyncCountIncreaseRequestDto request, MultipartFile image, String username) throws IOException {
        UserAuthModel user = userRepository.getUserDetailsByUsername(username).orElseThrow(() -> new ResourceNotFoundException(USER_NOT_FOUND));
        UserValidations.validateUserGmailSyncCountIncreaseRequest(request, user.getId(), gmailSyncRepository);
