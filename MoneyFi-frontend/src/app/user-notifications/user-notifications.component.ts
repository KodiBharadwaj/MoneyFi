...
export class UserNotificationsComponent implements OnInit {
  ...
  loadNotifications() {
    this.isLoading = true;
    this.http.get<UserNotification[]>(`${this.baseUrl}/api/v1/user/get-notifications`)
      .subscribe({
        next: (data) => {
  ...
  markSelectedAsRead() {
    ...
    this.http.put(`${this.baseUrl}/api/v1/user/user-notification/update?ids=${idsString}`, {})
      .subscribe({
        next: () => {
