...
export class RaiseRequestComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.unblockReasons = [];
    this.http.get<string[]>(`${this.baseUrl}/api/v1/user-service/open/reasons-dialog/get?code=4`).subscribe({
      next: (data) => {
    ...
    this.http.get<string[]>(`${this.baseUrl}/api/v1/user-service/open/reasons-dialog/get?code=3`).subscribe({
      next: (data) => {
    ...
    this.http.get<string[]>(`${this.baseUrl}/api/v1/user-service/open/reasons-dialog/get?code=6`).subscribe({
      next: (data) => {
  ...
  sendReference() {
    ...
    this.http.get<{ [key: string]: string }>(
      `${this.baseUrl}/api/v1/user-service/open/${backendRequestType}/${this.email}/reference-number-request`
    ).subscribe({
      next: (response) => {
  ...
  submitRequest() {
    ...
    if (this.selectedTab === 'rename') {
      ...
      this.http.post(`${this.baseUrl}/api/v1/user-service/open/name-change-request`, body).subscribe({
        next: (response) => {
      ...
      this.http.post(`${this.baseUrl}/api/v1/user-service/open/account-retrieve-request`, body).subscribe({
        next: (response) => {
          this.toastr.success('Request submitted successfully!');
          this.resetForm();
        },
