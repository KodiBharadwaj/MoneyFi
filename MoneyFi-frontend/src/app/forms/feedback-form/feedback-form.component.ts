...
export class FeedbackFormComponent {
  ...
  getNameAndEmailOfUser(){
    this.httpClient.get<ProfileDetails>(`${this.baseUrl}/api/v1/user-service/user/profile-details/get`).subscribe({
      next: (userProfile) => {
  ...
  submitFeedback() {
    if (this.feedback.name && this.feedback.email && this.feedback.rating) {
      ...
      this.httpClient.post(`${this.baseUrl}/api/v1/user-service/user/submit-feedback`, contactData).subscribe(
        (response) => {
